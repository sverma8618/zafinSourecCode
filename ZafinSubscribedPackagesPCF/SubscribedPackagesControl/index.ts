...
import {SubscribedPackages, IZafinConfigurationProps} from "./SubscribedPackages";

export class SubscribedPackagesControl
  ...
  async getFIProducts_SubscribedPackages(_customerId: string) {
  ...
  public init(
    ...
    this._configProps.ZafinUnSubcPckgUrl=context.parameters.ZafinUnsubcPckgUrl.raw?context.parameters.ZafinUnsubcPckgUrl.raw:"";
  ...
  public updateView(context: ComponentFramework.Context<IInputs>): void {
    ...
    var dataResponse=this.getFIProducts_SubscribedPackages(this._configProps.CustomerId);
    dataResponse.then(
      (response: ComponentFramework.WebApi.RetrieveMultipleResponse) => {
        ...
        if(response.entities.length>0)
        {
          ...
          this._configProps.SubscribedProductCodes=  productCodeResponse.replace(/,\s*$/, "");
          ReactDOM.render(
            React.createElement(SubscribedPackages,this._configProps),
            this._container
          );
        }
      },
